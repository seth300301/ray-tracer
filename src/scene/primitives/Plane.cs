...
namespace RayTracer
{
    ...
    public class Plane : SceneEntity
    {
        ...
        {
            // if the ray's direction and the plane's normal are not perpendiclar
            if (Math.Abs(ray.Direction.Dot(this.normal)) > Double.Epsilon) {
                double distance = ((this.center - ray.Origin).Dot(this.normal)) / (ray.Direction.Dot(this.normal));

                // if plane is infront of the ray
